...
namespace Reka.DebugConsole
{
	...
	public class Command : ScriptableObject
	{
		...
		public void Execute(string commandExtension, string[] args)
		{
			Debug.Log($"Executing command {commandExtension} \n with args: {string.Join(", ", args)}");
		}
		...
		public static string[] GetCommandArgs(string command)
		{
			...
			string[] filteredCommandArgs = new string[commandArgs.Length];
			...
			for (int i = 1; i < commandArgs.Length; i++)
			{
				if (commandArgs[i] == " ")
					continue ;
				filteredCommandArgs[i] = commandArgs[i];
			}
